		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  // JMCG: All PARSEC inputs are divisible, so we just check for sanity rather than including
  // code to process leftovers
#ifdef SIMD_WIDTH
  if((dim % SIMD_WIDTH) != 0) {
    printf("Error, Cannot be vectorized with SIMD (dim not divisible by SIMD width)\n");
    exit(-1);
  }
#endif
