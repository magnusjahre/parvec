...
const int SIMD_REG_SIZE = 8 * SIMD_WIDTH;
//typedef __m128i _MM_TYPE_I;
//typedef __m128 _MM_TYPE;
...
//#define _MM_LOAD  _mm_load_ps1
//#define _MM_STORE _mm_store_ps1
...
void print_mmxxxi(_MM_TYPE_I mm) {
    ...
    //_mmxxx_store_si((_MM_TYPE_I*)unpacked, mm);
    _MM_STORE(_mm_extract_epi32(unpacked, 0), _MM_CAST_I_TO_FP(mm));
...
static int searchDatabaseSW_(unsigned char query[], int queryLength,
    ...
    while (numEndedDbSeqs < dbLength) {
        ...
        for (unsigned char letter = 0; letter < alphabetLength; letter++) {
            ...
            //P[letter] = _mmxxx_load_si((_MM_TYPE_I const*)profileRow);
            P[letter] = _MM_CAST_FP_TO_I(_MM_LOAD((float const*)profileRow));
        }
        // ---------------------------------------------------------------------- //
        ...
        //_mmxxx_store_si((_MM_TYPE_I*)unpackedMaxH, maxH);
        _MM_STORE((float*)unpackedMaxH, _MM_CAST_I_TO_FP(maxH));
        ...
        if (!SIMD::satArthm) {
            ...
            //_mmxxx_store_si((_MM_TYPE_I*)unpackedOfTest, ofTest);
            _MM_STORE((float*)unpackedOfTest, _MM_CAST_I_TO_FP(ofTest));
            for (int i = 0; i < SIMD::numSeqs; i++) {
        ...
        if (searchType != OPAL_SEARCH_SCORE) {
            for (int i = 0; i < rowsWithImprovementLength; i++) {
                ...
                //_mmxxx_store_si((_MM_TYPE_I*)unpackedH, prevColumn[r].H);
                _MM_STORE((float*)unpackedH, _MM_CAST_I_TO_FP(prevColumn[r].H));
        ...
        if (overflowDetected || sequenceEnded) { // If at least one sequence ended
            ...
            //_MM_TYPE_I resetMaskPacked = _mmxxx_load_si((_MM_TYPE_I const*)resetMask);
            _MM_TYPE_I resetMaskPacked = _MM_CAST_FP_TO_I(_MM_LOAD((float const*)resetMask));
    ...
    while (numEndedDbSeqs < dbLength) {
        ...
        if (MODE == OPAL_MODE_NW) {
            if (seqJustLoaded) {
                ...
                //const _MM_TYPE_I resetMaskPacked = _mmxxx_load_si((_MM_TYPE_I const*)resetMask);
                const _MM_TYPE_I resetMaskPacked = _MM_CAST_FP_TO_I(_MM_LOAD((float const*)resetMask));
        ...
        if (!SIMD::satArthm) {
            ...
            //_mmxxx_store_si((_MM_TYPE_I*)unpackedMinEF, minEF);
            _MM_STORE((float*)unpackedMinEF, _MM_CAST_I_TO_FP(minEF));
            for (int i = 0; i < SIMD::numSeqs; i++) {
        ...
        if (searchType != OPAL_SEARCH_SCORE && (MODE == OPAL_MODE_HW || MODE == OPAL_MODE_OV)) {
            ...
            //_mmxxx_store_si((_MM_TYPE_I*)unpackedGreater, greater);
            _MM_STORE((float*)unpackedGreater, _MM_CAST_I_TO_FP(greater));
            for (int i = 0; i < SIMD::numSeqs; i++) {
        ...
        if (overflowDetected || shortestDbSeqLength == columnsSinceLastSeqEnd) { // If at least one sequence ended
            ...
            //_mmxxx_store_si((_MM_TYPE_I*)unpackedBestScore, bestScore);
            _MM_STORE((float*)unpackedBestScore, _MM_CAST_I_TO_FP(bestScore));
                        ...
                        if (!overflowed[i]) {
                            ...
                            if (searchType == OPAL_SEARCH_SCORE) {
                                ...
                                if (MODE == OPAL_MODE_OV) {
                                    ...
                                    //_mmxxx_store_si((_MM_TYPE_I*)unpackedPrevMaxLastRowH, prevMaxLastRowH);
                                    _MM_STORE((float*)unpackedPrevMaxLastRowH, _MM_CAST_I_TO_FP(prevMaxLastRowH));
                                    ...
                                    if (unpackedMaxH[i] > maxScore) {
                                        ...
                                        for (int r = 0; r < queryLength; r++) {
                                            ...
                                            //_mmxxx_store_si((_MM_TYPE_I*)unpackedPrevH, prevHs[r]);
                                            _MM_STORE((float*)unpackedPrevH, _MM_CAST_I_TO_FP(prevHs[r]));
            ...
            //const _MM_TYPE_I resetMaskPacked = _mmxxx_load_si((_MM_TYPE_I const*)resetMask);
            //const _MM_TYPE_I setMaskPacked = _mmxxx_load_si((_MM_TYPE_I const*)setMask);
            const _MM_TYPE_I resetMaskPacked = _MM_CAST_FP_TO_I(_MM_LOAD((float const*)resetMask));
            const _MM_TYPE_I setMaskPacked = _MM_CAST_FP_TO_I(_MM_LOAD((float const*)setMask));
